...
app = typer.Typer(help="Manage code review sessions.")
...
def start(
    ...
    try:
        with get_db() as db:
            ...
            session = svc.start(SessionType.REVIEW, ticket_id=ticket, pr_number=pr)
            console.print(f"[green]Review session started[/green] ({session.id})")
...
def end(
    ...
    try:
        with get_db() as db:
            svc = SessionService(db)
            session = svc.end()
            console.print(f"[green]Review session ended[/green] ({session.id})")
